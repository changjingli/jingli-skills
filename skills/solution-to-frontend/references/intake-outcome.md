# Intake and Outcome

Use this stage to transform solution material into a confirmed product intent. Do not design screens yet.

## Inputs

Inspect available proposals, presentations, spreadsheets, diagrams, contracts or scope notes, meeting records, existing systems, reference products, and repository evidence.

## Classify Evidence

Keep these categories visibly separate:

| Category | Meaning |
|---|---|
| Confirmed fact | Verified customer, business, platform, or data truth |
| Proposal | A recommended capability or direction, not yet approved |
| Demonstration content | Material intended to communicate an idea, not production truth |
| Assumption | A temporary belief with an owner and validation path |
| Open decision | A choice that materially changes scope or downstream work |

## Questions

Resolve only gaps that change the product materially:

- Who is the primary user or audience, in what situation?
- What must they understand or accomplish?
- What measurable or observable outcome makes the work valuable?
- What is inside the first delivery, explicitly outside it, and deferred?
- Which customer commitments, regulations, platforms, data sources, or assets are binding?

Do not ask the user to repeat facts strongly supported by project evidence. Present inferences for confirmation.

## Output

Produce:

1. one-sentence value goal;
2. audience and operating context;
3. business outcome and observable success;
4. scope, non-goals, dependencies, and risks;
5. evidence classification and open decisions;
6. candidate screens or flows only after the value goal is confirmed.

## Gate

Ask the accountable stakeholder to approve the value goal and scope. Do not infer approval from silence, an existing proposal, or runnable code.
